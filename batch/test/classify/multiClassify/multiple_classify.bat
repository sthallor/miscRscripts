...
:: Classify list of Drill Rig files in a single run
:: FHS, Mar 31, 2019
...
for %%f in (%filenames%) do (
	...
	echo/
	echo/
	echo Preparing rig classification parameters for file:
	echo %%f
	CALL "C:\Users\Fred.Seymour\Rcode\master\batch\test\classify\multiClassify\BatchSubstitute.bat" YYYYY %%f classifyYYYYY.prm > classify.prm
	echo 'Running the rig state classifier program'
	CALL "C:\Program Files\R\R-3.4.2\bin\x64\Rscript.exe"^
	 /Users/Fred.Seymour/Rcode/master/source/classify/mainClassify.R^
	 /Users/Fred.Seymour/Rcode/master/batch/test/classify/multiClassify/classify.prm
)
