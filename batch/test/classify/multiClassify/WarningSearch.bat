...
set keywords="WARNING"
set outfile="WARNING.txt"
...
for /f "tokens=*" %%G in ('dir /b /s /a:d %dirname%') do if exist "%%G%filefilter%" (
   ...
   if !count!==0 (
      echo No WARNINGS Found in "%%G%filefilter%"
   ) ELSE (      
      echo Found !count! WARNINGS in "%%G%filefilter%"
      echo Found !count! WARNINGS in "%%G%filefilter%" >> %outfile%
