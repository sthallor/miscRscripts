...
impute <- function(dt.col,col.name,prm.dc,convertCount) {
  ...
  if (col.name %in% prm.dc$valueLimits$name) {
    ...
    cci <- which(convertCount$name %in% col.name) # convertCount index
    maxLook <- 999 # non-zero pass through value, maxLook only used in continuous case, FHS Feb 22, 2019
  } else {
  ...
  # Check if there are no NAs for interpolation/extrapolation
  # Also, if maxLook less than or equal to zero, returns with no interpolation/extrapolation, FHS Feb 22, 2019
  if (sum(is.na(dt.col))==0 | maxLook <= 0) {
    # update counter and return
    convertCount$totalOut[cci] <- convertCount$totalOut[cci] + length(dt.col)
    convertCount$endingNA[cci] <- convertCount$endingNA[cci] + sum(is.na(dt.col))
